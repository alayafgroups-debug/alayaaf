...
import {
  Bell, BriefcaseBusiness, Calculator, CheckCircle, ChevronDown, Languages,
  LogIn, LogOut, Mail, MessageSquare, Phone, RefreshCw, User, X,
} from "lucide-react";
...
const actions = [
  { id: "contact" as const, label: "اتصل بنا", icon: Phone, color: "from-sky-500 to-blue-600" },
  { id: "management" as const, label: "تواصل مع الإدارة", icon: MessageSquare, color: "from-indigo-500 to-violet-600" },
  { id: "refresh" as const, label: "تحديث الكاش", icon: RefreshCw, color: "from-emerald-500 to-teal-600" },
  { id: "calculator" as const, label: "آلة حاسبة", icon: Calculator, color: "from-amber-500 to-orange-600" },
  { id: "incoming" as const, label: "الطلبات الواردة", icon: Mail, color: "from-cyan-500 to-sky-600" },
  { id: "notifications" as const, label: "الإشعارات", icon: Bell, color: "from-rose-500 to-pink-600" },
  { id: "language" as const, label: "اللغة", icon: Languages, color: "from-purple-500 to-fuchsia-600" },
  { id: "tasks" as const, label: "المهام والمشاريع", icon: BriefcaseBusiness, color: "from-slate-600 to-slate-800" },
];
...
export default function QuickActionsBar() {
  ...
  const loadNotices = async () => {
    const [leaveResult, requestResult] = await Promise.all([
      supabase.from("leave_requests").select("id, leave_type, status, created_at").order("created_at", { ascending: false }).limit(5),
      supabase.from("hr_requests").select("id, request_type, status, created_at").order("created_at", { ascending: false }).limit(5),
    ]);
    const rows: Notice[] = [
      ...(leaveResult.data ?? []).map((row: any) => ({ id: `l-${row.id}`, title: row.leave_type ?? "طلب إجازة", status: row.status ?? "معلق", date: row.created_at ?? "" })),
      ...(requestResult.data ?? []).map((row: any) => ({ id: `r-${row.id}`, title: row.request_type ?? "طلب موظف", status: row.status ?? "معلق", date: row.created_at ?? "" })),
    ].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 7);
    setNotices(rows);
    setNoticeCount(rows.filter((row) => ["معلق", "معلقة", "pending"].includes(row.status)).length);
  };

  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from("app_contact_settings").select("phone, email, whatsapp").eq("id", "main").maybeSingle();
      if (data) setContacts({ phone: data.phone ?? "", email: data.email ?? "", whatsapp: data.whatsapp ?? "" });
      await loadNotices();
    };
    void load();
  }, []);
  ...
  const result = operator === "+" ? firstNumber + secondNumber
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-thin">
        {actions.map(({ id, label, icon: Icon, color }) => (
          <button key={id} onClick={() => handleAction(id)} className="group relative flex min-w-[76px] flex-col items-center gap-1 rounded-xl px-2 py-1.5 hover:bg-slate-50">
            <span className={`relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br ${color} text-white shadow-sm transition group-hover:-translate-y-0.5 group-hover:shadow-md`}>
