  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      <div className="flex items-center gap-3 p-4 bg-white border-b sticky top-0 z-10">
        ...
        <h2 className="font-bold text-lg text-gray-900">من نحن</h2>
      </div>
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-4">
        {/* Company hero */}
        <div className="bg-gradient-to-l from-[#004e89] to-[#0066b3] rounded-xl p-6 text-white">
          <Building2 className="h-10 w-10 mb-3 text-white/80" />
          <h3 className="text-xl font-bold mb-1">شركة الأياف</h3>
          <p className="text-blue-100 text-sm">نظام إدارة الموارد البشرية المتكامل</p>
        </div>

        {/* Branches */}
        <div>
          <p className="font-semibold text-gray-700 text-sm mb-2 px-1">مقرات الشركة والفروع</p>
          {loading ? (
            <div className="text-center py-8 text-gray-400 text-sm">جاري التحميل...</div>
          ) : branches.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 text-center text-gray-400 text-sm">لا توجد فروع مسجلة</div>
          ) : branches.map((b) => (
