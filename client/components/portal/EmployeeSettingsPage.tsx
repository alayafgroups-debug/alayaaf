  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-4">
        ...
        {loading ? (
          <div className="text-center py-8 text-gray-400">جاري تحميل الإعدادات...</div>
        ) : (
          <>
            {/* Language */}
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => save({ ...prefs, lang: "ar" })} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold border transition ${prefs.lang === "ar" ? "bg-[#004e89] text-white border-[#004e89]" : "border-gray-200 text-gray-600"}`}>العربية</button>
                <button onClick={() => save({ ...prefs, lang: "en" })} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold border transition ${prefs.lang === "en" ? "bg-[#004e89] text-white border-[#004e89]" : "border-gray-200 text-gray-600"}`}>English</button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <div className="flex items-center justify-between">
                <p className="font-semibold text-gray-800 flex items-center gap-2">
                  ...
                  الوضع {prefs.darkMode ? "الليلي" : "النهاري"}
                </p>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              ...
              <p className="text-xs text-gray-400 mt-2">{prefs.notifications ? "الإشعارات مفعّلة" : "الإشعارات متوقفة"}</p>
            </div>
          </>
        )}
