    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3 p-4 bg-white border-b sticky top-0 z-10">
          ...
          <h2 className="font-bold text-lg text-gray-900">تفاصيل الطلب</h2>
        </div>
        <div className="flex-1 overflow-y-auto p-4 pb-24 bg-gray-50">
          <div className="bg-white rounded-xl shadow-sm p-5 mb-4">
            <div className="flex justify-between items-start mb-4">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColor(selected.status)}`}>{selected.status}</span>
          ...
          {isPending && (
            <div className="flex gap-3">
              <button onClick={() => handleReject(selected)} disabled={processing} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-red-500 text-red-500 font-bold text-sm">
                {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-5 w-5" />} رفض
              </button>
              <button onClick={() => handleApprove(selected)} disabled={processing} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#004e89] text-white font-bold text-sm">
                {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-5 w-5" />} موافقة
              </button>
            </div>
          )}
        </div>
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="flex items-center gap-3 p-4">
          ...
          <h2 className="font-bold text-lg text-gray-900">الطلبات</h2>
        </div>
        <div className="flex border-b">
          {[{ key: "pending", label: "معلقة" }, { key: "incoming", label: "الواردة" }, { key: "sent", label: "المرسلة" }].map(({ key, label }) => (
            <button key={key} onClick={() => setTab(key as any)} className={`flex-1 py-3 text-sm font-semibold border-b-2 transition ${tab === key ? "border-[#004e89] text-[#004e89]" : "border-transparent text-gray-500"}`}>{label}</button>
          ))}
        </div>
          ...
          filtered.map((req, i) => (
            <div key={req.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-3">
              <div className="flex justify-between items-center mb-2">
                <p className="text-gray-400 text-xs">{req.date}</p>
                <p className="text-gray-500 text-xs">#{1180 + i}</p>
              </div>
              <div className="space-y-1 mb-3">
                <div className="flex justify-between text-sm"><span className="text-gray-500">الاسم</span><span className="font-semibold">{req.empName}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">نوع الطلب</span><span>{req.type}</span></div>
                <div className="flex justify-center"><span className={`px-3 py-0.5 rounded-full text-xs font-medium ${statusColor(req.status)}`}>{req.status}</span></div>
              </div>
