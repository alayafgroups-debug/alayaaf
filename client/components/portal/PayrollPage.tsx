    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3 p-4 bg-white border-b sticky top-0 z-10">
          ...
          <h2 className="font-bold text-lg text-gray-900">قسيمة الراتب — {selected.month}</h2>
        </div>
        <div className="flex-1 overflow-y-auto p-4 pb-24 bg-gray-50">
          <div className="bg-[#004e89] text-white rounded-xl p-5 mb-4 text-center">
            <p className="text-blue-200 text-sm mb-1">صافي الراتب</p>
            <p className="text-4xl font-bold">{(+selected.net_salary).toLocaleString("ar-SA")} <span className="text-xl">ر.س</span></p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 space-y-3">
            {[
              ...
              <div key={label} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                ...
                <span className={`font-semibold ${color}`}>{negative ? "-" : "+"}{(+value).toLocaleString("ar-SA")} ر.س</span>
              </div>
            ))}
          </div>
          <div className="bg-white rounded-xl shadow-sm p-5 mt-4">
            <div className="flex justify-between items-center">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${selected.status === "مدفوع" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{selected.status}</span>
            </div>
            {selected.paid_date && <div className="flex justify-between items-center mt-2"><span className="text-gray-500 text-sm">تاريخ الدفع</span><span className="text-gray-800 text-sm font-medium">{selected.paid_date}</span></div>}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 p-4 bg-white border-b sticky top-0 z-10">
        ...
        <h2 className="font-bold text-lg text-gray-900">حساب الراتب</h2>
      </div>
      <div className="flex-1 overflow-y-auto p-4 pb-24 bg-gray-50">
        {loading ? (
          <div className="text-center py-12 text-gray-400">جاري التحميل...</div>
        ) : records.length === 0 ? (
          <div className="text-center py-12 text-gray-400">لا توجد سجلات رواتب</div>
        ) : (
          records.map((rec) => (
            <button key={rec.id} onClick={() => setSelected(rec)} className="w-full bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-3 flex items-center gap-4 hover:shadow-md transition text-right">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">صافي: {(+rec.net_salary).toLocaleString("ar-SA")} ر.س</p>
              </div>
              <div className="text-left">
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${rec.status === "مدفوع" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{rec.status}</span>
              </div>
