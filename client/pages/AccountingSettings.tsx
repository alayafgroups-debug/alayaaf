    ...
    <Layout>
      <main dir={direction} className="space-y-5">
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><div><h2 className="font-bold">{t("الفترات المالية")}</h2><p className="text-xs text-slate-500">{t("الترحيل المحاسبي وحركات المخزون متاحان فقط داخل فترة مفتوحة. إغلاق الفترة لا يمنع حفظ مسودات القيود.")}</p></div><button onClick={startNew} className="inline-flex items-center gap-2 rounded bg-indigo-700 px-4 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" />{t("فترة مالية جديدة")}</button></div>
