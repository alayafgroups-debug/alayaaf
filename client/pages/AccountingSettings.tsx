    ...
    <Layout>
      <main dir={direction} className="space-y-5">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {loading ? <div className="py-10 text-center">{t("جاري التحميل...")}</div> : <><div className="grid gap-4 md:grid-cols-3"><AccountSelect label="ذمم العملاء" field="receivableAccountCode" prefix="1" /><AccountSelect label="إيرادات المبيعات" field="revenueAccountCode" prefix="4" /><AccountSelect label="ضريبة المخرجات" field="outputVatAccountCode" prefix="2" /><AccountSelect label="ذمم الموردين" field="payableAccountCode" prefix="2" /><AccountSelect label="المشتريات والمصروفات" field="purchaseAccountCode" prefix="5" /><AccountSelect label="ضريبة المدخلات" field="inputVatAccountCode" prefix="2" /><AccountSelect label="مصروف عجز المخزون" field="inventoryShortageAccountCode" prefix="5" /><AccountSelect label="إيراد فائض المخزون" field="inventorySurplusAccountCode" prefix="4" /></div><div className="mt-4 flex justify-end"><button onClick={() => void saveDefaults()} disabled={busy || !canManage} className="inline-flex items-center gap-2 rounded bg-indigo-700 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{t("حفظ إعدادات الترحيل")}</button></div></>}
        </section>
