...
export function AttendanceMonthlyReportContent({ embedded = false }: { embedded?: boolean }) {
  ...
  const visibleData = useMemo(() => {
    ...
    return data.filter((employee) => employee.empName.toLowerCase().includes(query) || employee.empId.toLowerCase().includes(query));
  }, [data, employeeSearch]);
  const countStatus = (statuses: string[]) => visibleData.reduce((total, employee) => total + days.filter((day) => statuses.includes(employee.attendance[day]?.status)).length, 0);
  ...
  const reportRows = visibleData.map((employee) => ({ empId: employee.empId, empName: employee.empName, department: employee.department, section: employee.section, ...Object.fromEntries(days.map((day) => [`day${day}`, t(employee.attendance[day]?.status || "-")])) }));
  const monthLabel = new Date(year, month - 1).toLocaleString(localeCode, { month: "long", year: "numeric" });
  const reportSummary = [{ label: t("عدد الموظفين"), value: formatNumber(visibleData.length) }, { label: t("إجمالي الحضور"), value: formatNumber(countStatus(["حاضر"])) }, { label: t("إجمالي الغياب"), value: formatNumber(countStatus(["غياب"])) }];
