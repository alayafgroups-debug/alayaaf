...
type MonthlyAttendance = { empId: string; empName: string; department: string; section: string; attendance: Record<number, { status: string; notes: string }> };
const ALL = "الكل";
...
export default function HRAttendanceMonthlyReport() {
  ...
  const loadDepartmentsAndSections = async () => {
    try {
      const { data: emps } = await supabase.from("employees").select("department, division").in("status", ["نشط", "فعال"]);
      if (emps) { setDepartments([...new Set(emps.map((employee: any) => employee.department).filter(Boolean))] as string[]); setSections([...new Set(emps.map((employee: any) => employee.division).filter(Boolean))] as string[]); }
    } catch (error) { console.error("Error loading departments:", error); }
  };
  const loadData = async () => {
    setLoading(true);
    try {
      const { data: emps } = await supabase.from("employees").select("id, emp_id, name, department, division").in("status", ["نشط", "فعال"]);
      ...
      let result = emps.map((employee: any) => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
          ...
          attendance[day] = weekday === 5 || weekday === 6 ? { status: "عطلة نهاية أسبوع", notes: "" } : attMap[employee.emp_id]?.[day] || { status: "غياب", notes: "" };
        }
        return { empId: employee.emp_id || "-", empName: employee.name || "-", department: employee.department || "-", section: employee.division || "-", attendance };
      });
      if (departmentFilter !== ALL) result = result.filter((row) => row.department === departmentFilter);
      if (sectionFilter !== ALL) result = result.filter((row) => row.section === sectionFilter);
  ...
  useEffect(() => { void loadData(); }, [year, month, departmentFilter, sectionFilter]);
  ...
  const getStatusColor = (status: string) => ({ "حاضر": "bg-green-100 text-green-700", "غياب": "bg-red-100 text-red-700", "إجازة": "bg-blue-100 text-blue-700", "مأمورية": "bg-purple-100 text-purple-700", "متأخر": "bg-yellow-100 text-yellow-700", "عطلة نهاية أسبوع": "bg-gray-100 text-gray-700" }[status] || "bg-gray-100 text-gray-700");
  ...
  </div><style>{`@media print { body { margin: 0; } .no-print { display: none; } table { font-size: 10px; } th, td { padding: 4px !important; } }`}</style></Layout>;
}
