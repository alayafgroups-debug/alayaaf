...
import { AttendanceMonthlyReportContent } from "@/pages/HRAttendanceMonthlyReport";
...
export default function HRAttendancePreparation() {
  ...
  const [activeView, setActiveView] = useState<"preparation" | "monthly">("preparation");
  ...
  return <Layout><div className="mx-auto max-w-[1800px] space-y-5" dir={direction}>
    <div className="rounded-xl border bg-white shadow-sm">
      <div className="border-b px-5 py-4"><h1 className="text-xl font-bold text-slate-900">{t("حساب الدوام")}</h1></div>
      <div className="flex flex-wrap justify-end gap-1 p-3">
        <button type="button" onClick={() => setActiveView("preparation")} className={`rounded-md px-5 py-2.5 text-sm font-semibold transition ${activeView === "preparation" ? "bg-[#0069a8] text-white shadow-sm" : "text-slate-700 hover:bg-slate-100"}`}>{t("تقرير حساب دوام الموظفين")}</button>
        <button type="button" onClick={() => setActiveView("monthly")} className={`rounded-md px-5 py-2.5 text-sm font-semibold transition ${activeView === "monthly" ? "bg-[#0069a8] text-white shadow-sm" : "text-slate-700 hover:bg-slate-100"}`}>{t("الحضور والغياب للموظفين")}</button>
      </div>
    </div>
    {activeView === "monthly" ? <AttendanceMonthlyReportContent embedded /> : <>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-slate-900">{t("تقرير حساب دوام الموظفين")}</h2><p className="mt-1 text-sm text-slate-500">{t("تسجيل حضور موظف واحد أو مجموعة موظفين وربطه مباشرة بتقارير الحضور")}</p></div><div className="flex items-center gap-2">{canManageLocations && <Button type="button" variant="outline" onClick={() => setShowLocationManager((current) => !current)} className="gap-2 border-violet-200 text-violet-700 hover:bg-violet-50"><MapPin className="h-4 w-4" />{t("موقع الحضور")}</Button>}<div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-800">{t("المحددون")}: {formatNumber(selected.size)}</div></div></div>
