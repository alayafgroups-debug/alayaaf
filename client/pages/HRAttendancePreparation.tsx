...
type Employee = { id: string; empId: string; name: string; department: string; branch: string; workLocation: string; workSchedule: string; attendanceLocationId: string };
...
export default function HRAttendancePreparation() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [employeeResult, scheduleResult, locationResult] = await Promise.all([
        supabase.from("employees").select("id, emp_id, name, department, branch, work_location, work_schedule, attendance_location_id").in("status", ["نشط", "فعال", "active"]).order("name"),
      ...
      if (employeeResult.error || scheduleResult.error || locationResult.error) toast.error(t("تعذر تحميل بيانات الموظفين وجداول العمل"));
      setEmployees((employeeResult.data ?? []).map((row: any) => ({ id: String(row.id), empId: String(row.emp_id ?? row.id), name: String(row.name ?? "-"), department: String(row.department ?? "غير محدد"), branch: String(row.branch ?? "غير محدد"), workLocation: String(row.work_location ?? "غير محدد"), workSchedule: String(row.work_schedule ?? "بدون جدول عمل"), attendanceLocationId: String(row.attendance_location_id ?? "") })));
      setSchedules((scheduleResult.data ?? []).map((row: any) => ({ id: String(row.id), name: String(row.name), hours: String(row.hours ?? "") })));
      const mappedLocations = (locationResult.data ?? []).map((row: any) => ({ id: String(row.id), name: String(row.name ?? ""), address: String(row.address ?? ""), latitude: row.latitude == null ? null : Number(row.latitude), longitude: row.longitude == null ? null : Number(row.longitude), radius: Number(row.attendance_radius_m ?? 10), isDefault: Boolean(row.is_company_default) }));
  ...
  const options = (field: "branch" | "department" | "workLocation") => [...new Set(employees.map((employee) => employee[field]))].sort();
  ...
  const handlePrepare = async () => {
    ...
    const payload = dates.flatMap((attendanceDate) => chosen.map((employee) => ({ emp_id: employee.empId, emp_name: employee.name, department: employee.department, date: attendanceDate, check_in: status === "غائب" ? null : checkIn || null, check_out: status === "غائب" ? null : checkOut || null, status, late_minutes: 0, notes: notes.trim() || (chosen.length > 1 ? "تحضير جماعي من الإدارة" : "تحضير فردي من الإدارة"), entry_source: "manager_manual", prepared_by: session?.name || session?.email || "الإدارة", schedule_id: scheduleId || null, updated_at: new Date().toISOString() })));
  ...
  return <Layout><div className="mx-auto max-w-[1500px] space-y-5" dir={direction}>
...
function Filter({ value, onChange, values, label }: { value: string; onChange: (value: string) => void; values: string[]; label: string }) { return <select value={value} onChange={(e) => onChange(e.target.value)} aria-label={label} className="h-10 rounded-md border bg-white px-3 text-sm"><option value="الكل">{label}</option>{values.map((item) => <option key={item} value={item}>{item}</option>)}</select>; }
