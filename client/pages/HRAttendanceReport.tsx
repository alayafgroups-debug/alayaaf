...
type Employee = { id: string; empId: string; name: string; branch: string; administration: string; departmentId: string; department: string; workLocation: string; workSchedule: string; workTime: string; dailyHours: number };
...
export default function HRAttendanceReport() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [{ data: employeeRows, error: employeesError }, { data: attendanceRows, error: attendanceError }, { data: departmentRows, error: departmentsError }] = await Promise.all([
        supabase.from("employees").select("id, emp_id, name, branch, division, department_id, directorate, department, work_location, work_schedule, work_time, daily_hours").in("status", ["نشط", "فعال", "active"]).order("name"),
      ...
      if (employeesError) throw employeesError; if (attendanceError) throw attendanceError; if (departmentsError) throw departmentsError;
      const departmentOptions = (departmentRows ?? []).map((row) => ({ id: String(row.id), name: String(row.name) }));
      const departmentById = new Map(departmentOptions.map((item) => [item.id, item.name]));
      setDepartments(departmentOptions);
      setEmployees((employeeRows ?? []).map((row: any) => ({ id: String(row.id), empId: String(row.emp_id ?? row.id), name: String(row.name ?? "-"), branch: String(row.branch ?? t("غير محدد")), administration: String(row.division ?? t("غير محدد")), departmentId: String(row.department_id ?? ""), department: departmentById.get(String(row.department_id ?? "")) || String(row.directorate ?? row.department ?? t("غير محدد")), workLocation: String(row.work_location ?? t("غير محدد")), workSchedule: String(row.work_schedule ?? t("بدون جدول عمل")), workTime: String(row.work_time ?? t("دوام كامل")), dailyHours: Number(row.daily_hours ?? 0) })));
  ...
  const filteredEmployees = employees.filter((employee) => (branch === ALL || employee.branch === branch) && (administration === ALL || employee.administration === administration) && (department === ALL || employee.departmentId === department || (!employee.departmentId && employee.department === departments.find((item) => item.id === department)?.name)) && (workLocation === ALL || employee.workLocation === workLocation) && (workTime === ALL || employee.workTime === workTime));
