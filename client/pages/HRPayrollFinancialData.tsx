      ...
      <div className="p-6 max-w-[1600px] mx-auto space-y-6" dir="rtl">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-[#004e89] text-white p-3 flex items-center justify-between gap-4">
            <h2 className="text-lg font-bold">البيانات المالية للموظفين</h2>
            <div className="flex items-center gap-2">
              <button className="p-1.5 hover:bg-white/10 rounded transition-colors text-white" title="تصدير"><Download className="h-4 w-4" /></button>
              <button className="p-1.5 hover:bg-white/10 rounded transition-colors text-white" title="سجل"><FileText className="h-4 w-4" /></button>
            </div>
          </div>
            ...
            <table className="w-full text-sm text-right whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="py-8 text-center text-gray-500">لا يوجد موظفون</td></tr>
                ) : employees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4">{emp.workTime || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
