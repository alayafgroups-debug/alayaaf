    ...
    <Layout>
      <div className="p-6 max-w-[1600px] mx-auto space-y-6" dir={direction}>
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">{t("مخالصة الذمة للموظفين")}</h1>
        </div>
            ...
            <table className="w-full text-sm text-right min-w-[1200px]">
              <thead className="bg-[#004e89] text-white">
                <tr>
                  ...
                  <th className="py-3 px-4 font-medium">{t("تاريخ إنهاء الخدمة")}</th>
                  <th className="py-3 px-4 font-medium">{t("المبلغ")}</th>
                  <th className="py-3 px-4 font-medium text-center">{t("رد الموظف")}</th>
                  <th className="py-3 px-4 font-medium">{t("تاريخ الرد")}</th>
              ...
              <tbody className="divide-y bg-white">
                {loading ? (
                  <tr><td colSpan={9} className="text-center py-8 text-gray-400">{t("جاري التحميل...")}</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={9} className="text-center py-8 text-gray-400">{t("لا توجد بيانات")}</td></tr>
                ) : filtered.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="py-3 px-4 text-center"><span className={`px-2 py-0.5 rounded-full text-xs ${row.employeeReply === "موافق عليه" ? "bg-emerald-100 text-emerald-800" : "bg-yellow-100 text-yellow-800"}`}>{t(row.employeeReply)}</span></td>
