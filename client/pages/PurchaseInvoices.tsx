...
export default function PurchaseInvoices() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!error) {
      ...
      toast({ title: t("تم حذف الفاتورة"), description: `${t("الفاتورة")}: ${id}` });
    } else {
        ...
        <body>
          <main class="sheet">
            <div class="company-row">
              <div class="company-ar"><strong>${escapeHtml(t("شركة لاكجري العياف"))}</strong><br>${escapeHtml(t("8529 الشيخ محمد بن جبير، الشوقية، مكة المكرمة"))}<br>${escapeHtml(t("الرقم الضريبي"))} 314559705300003<br>${escapeHtml(t("السجل التجاري"))} 7053358979</div>
            ...
            <div class="bottom">
              <div class="notes"><strong>${escapeHtml(t("ملاحظات"))} / Notes</strong><br>${escapeHtml(invoice.notes || t("لا توجد ملاحظات"))}</div>
              <div class="totals">
            ...
            <div class="footer">${escapeHtml(t("شركة لاكجري العياف"))} | Luxury Al Ayaf Company</div>
          </main>
        </body>
      </html>
    `);
        ...
        {view === "create" && (
          <InvoiceForm
            ...
            onSaved={(inv) => {
              ...
              toast({ title: t("تم حفظ الفاتورة"), description: `${t("الفاتورة")}: ${inv.id}` });
              setView("list");
            }}
          />
        )}
...
function InvoicesList({
  ...
  const formatAmount = (value: string) => formatNumber(parseCurrency(value), { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const displayDate = (value: string) => value ? formatDate(value, { dateStyle: "medium" }) : "-";
  ...
  return (
    <div className="space-y-6" dir={direction}>
      ...
      <FilterBar>
        <FilterInput placeholder={`${t("رقم الفاتورة")}، ${t("المرجع")}، ${t("المورد")}...`} />
      ...
      <div className="rounded-2xl bg-white border border-border/50 shadow-sm overflow-x-auto">
        <table className="w-full text-sm text-right">
          <thead className={cn("text-white bg-gradient-to-r", "from-purple-800 to-indigo-900")}>
            <tr>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("الإجراءات")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("الحالة")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("المتبقي")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("المدفوع")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("الإجمالي")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("المورد")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("تاريخ الاستحقاق")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("تاريخ الفاتورة")}</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap text-right">{t("رقم الفاتورة")}</th>
            </tr>
          </thead>
          <tbody>
            {invoices.map((inv) => (
              <tr key={inv.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                <td className="px-4 py-3 align-middle">
                  <div className="flex items-center gap-1 flex-wrap">
                    <ActionBtn icon={Eye} label={t("عرض")} color="blue" onClick={() => onView(inv)} />
                    <ActionBtn icon={Edit} label={t("تعديل")} color="emerald" onClick={() => onEdit(inv)} />
                    <ActionBtn icon={CreditCard} label={t("تسديد")} color="indigo" onClick={() => onPayment(inv)} />
                    <ActionBtn icon={Trash2} label={t("حذف")} color="red" onClick={() => onDelete(inv.id)} />
                    <button
                      title={t("طباعة PDF")}
                ...
                <td className="px-4 py-3 align-middle text-red-600 font-medium whitespace-nowrap">{formatAmount(inv.remaining)} {t("ريال")}</td>
                <td className="px-4 py-3 align-middle text-green-700 font-medium whitespace-nowrap">{formatAmount(inv.paid)} {t("ريال")}</td>
                <td className="px-4 py-3 align-middle font-semibold whitespace-nowrap">{formatAmount(inv.total)} {t("ريال")}</td>
                <td className="px-4 py-3 align-middle whitespace-nowrap">{inv.vendor}</td>
                <td className="px-4 py-3 align-middle text-muted-foreground whitespace-nowrap">{displayDate(inv.dueDate)}</td>
                <td className="px-4 py-3 align-middle text-muted-foreground whitespace-nowrap">{displayDate(inv.date)}</td>
                <td
    ...
    <div className="space-y-6 bg-slate-50 min-h-screen pb-12" dir={direction}>
      <div className="flex justify-between items-center bg-white p-4 border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold text-slate-800">{t("تفاصيل فاتورة المشتريات")}</h1>
      ...
      <div className="p-4 space-y-6">
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-5">
            <div className="text-right text-xs leading-6 text-slate-600">
              <div className="text-base font-bold text-slate-900">{t("شركة لاكجري العياف")}</div>
              <div>{t("8529 الشيخ محمد بن جبير، الشوقية، مكة المكرمة")}</div>
              <div>{t("الرقم الضريبي")} 314559705300003</div>
            </div>
            <img src={COMPANY_LOGO_URL} alt={t("شعار شركة لاكجري العياف")} className="h-24 w-36 object-contain mx-auto" />
          ...
          <h2 className="mt-3 text-center text-2xl font-bold text-slate-900">{t("فاتورة مشتريات")} | Purchase Invoice</h2>
        </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { label: t("تاريخ الاستحقاق"), value: displayDate(invoice.dueDate) },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: t("الإجمالي"), value: `${formatAmount(parseCurrency(invoice.total))} ${t("ريال")}`, color: "text-slate-800", bg: "bg-slate-50" },
            { label: t("المدفوع"), value: `${formatAmount(parseCurrency(invoice.paid))} ${t("ريال")}`, color: "text-green-700", bg: "bg-green-50" },
            { label: t("المتبقي"), value: `${formatAmount(parseCurrency(invoice.remaining))} ${t("ريال")}`, color: "text-red-600", bg: "bg-red-50" },
          ...
          <div className="p-4 overflow-x-auto">
            <table className="w-full text-sm text-right">
              <thead className="bg-slate-100">
                <tr>
                  ...
                  <th className="px-3 py-2 border border-slate-200">{t("سعر الوحدة")}</th>
                ...
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2 border border-slate-200">{formatNumber(qty)}</td>
                      <td className="px-3 py-2 border border-slate-200">{formatAmount(price)}</td>
                      <td className="px-3 py-2 border border-slate-200">{formatAmount(disc)}</td>
                      <td className="px-3 py-2 border border-slate-200">{formatNumber(taxPct)}%</td>
                      <td className="px-3 py-2 border border-slate-200 font-medium">{formatAmount(sub + tax)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <div className="flex justify-end mt-6">
              <div className="w-80 space-y-2 text-sm">
                <div className="flex justify-between"><span className="font-semibold">{formatAmount(totals.subtotal)} {t("ريال")}</span><span className="text-slate-600">{t("المجموع الفرعي")}</span></div>
                <div className="flex justify-between"><span className="font-semibold">{formatAmount(totals.discount)} {t("ريال")}</span><span className="text-slate-600">{t("الخصم")}</span></div>
                <div className="flex justify-between"><span className="font-semibold">{formatAmount(totals.tax)} {t("ريال")}</span><span className="text-slate-600">{t("ضريبة القيمة المضافة")}</span></div>
                <div className="flex justify-between border-t border-slate-200 pt-2">
                  <span className="font-bold text-blue-600 text-base">{formatAmount(totals.total)} {t("ريال")}</span>
...
function ItemsTable({
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden" dir={direction}>
      <div className="bg-slate-100 px-4 py-2 text-right font-semibold text-slate-700 flex items-center justify-between">
        ...
        <button type="button" onClick={onAdd} className="inline-flex items-center gap-1 rounded bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">
          <Plus className="h-3.5 w-3.5" /> {t("إضافة بند")}
        </button>
      </div>
      <div className="p-4 overflow-x-auto">
        <div className="mb-3 text-sm text-slate-600 flex justify-end gap-6">
          <label className="inline-flex items-center gap-2"><input type="radio" name="tax-mode" defaultChecked /> {t("خالٍ من الضريبة")}</label>
          <label className="inline-flex items-center gap-2"><input type="radio" name="tax-mode" /> {t("شامل الضريبة")}</label>
        </div>
        {items.length === 0 ? (
          ...
          <table className="w-full text-sm text-right mb-4">
            <thead>
              <tr className="text-slate-600 border-b border-slate-200">
                ...
                <th className="pb-2 font-medium w-24">{t("حساب المصروفات")}*</th>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <tr key={`item-${idx}`}>
                    <td className="pt-3 align-top">
                      <div className="flex items-center justify-center h-10">
                        <button onClick={() => onRemove(item.id)} className="w-7 h-7 flex items-center justify-center bg-red-500 text-white rounded hover:bg-red-600">
                          <Trash2 className="w-3.5 h-3.5" aria-label={t("حذف البند")} />
                        </button>
                      </div>
                    </td>
                    <td className="pt-3 px-1 align-top">
                      <input type="text" value={formatAmount(lineTotal)} disabled aria-label={t("المجموع")} className="w-full px-2 py-2 border border-slate-200 bg-slate-100 rounded text-sm text-right outline-none h-10" />
                    </td>
                    <td className="pt-3 px-1 align-top">
                    ...
                    <td className="pt-3 px-1 align-top">
                      <input type="text" value={item.unit} onChange={(e) => onUpdate(item.id, { unit: e.target.value })} placeholder={t("مطلوب")} className={inputClass} />
                    </td>
        ...
        <div className="border-t border-slate-200 pt-4 mt-2 flex justify-end">
          <div className="w-80 space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t border-slate-200 pt-2">
              <span className="font-bold text-blue-600 text-base">{formatAmount(totals.total)} {t("ريال")}</span>
              <span className="font-bold text-slate-800">{t("المجموع الكلي")}</span>
            </div>
          </div>
        </div>
      </div>
...
function FormFields({
  ...
  return (
    <div className="p-6 grid grid-cols-1 md:grid-cols-[170px_1fr] gap-3 items-center" dir={direction}>
      <label className="text-sm font-medium text-slate-700">{t("رقم الفاتورة")}*</label>
      <input type="text" value={invoiceNumber || t("تلقائي")} disabled className="w-full h-10 px-3 py-2 border border-slate-200 bg-slate-100 rounded text-sm text-right outline-none text-slate-500" />

      <label className="text-sm font-medium text-slate-700">{t("المورد")}*</label>
      <input value={form.vendor} onChange={(e) => setField("vendor", e.target.value)} placeholder={t("مطلوب")} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("العملة")}*</label>
      ...
      <label className="text-sm font-medium text-slate-700">{t("التاريخ")}*</label>
      <input type="date" value={form.date} onChange={(e) => setField("date", e.target.value)} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("تاريخ الاستحقاق")}*</label>
      <input type="date" value={form.dueDate} onChange={(e) => setField("dueDate", e.target.value)} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("أمر شراء")}</label>
      <input value={form.poNumber} onChange={(e) => setField("poNumber", e.target.value)} placeholder={t("اختياري")} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("المرجع")}</label>
      <input value={form.referenceNo} onChange={(e) => setField("referenceNo", e.target.value)} placeholder={t("اختياري")} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("فرع")}</label>
      <input value={form.status} onChange={(e) => setField("status", e.target.value)} placeholder={t("اختياري")} className={inputClass} />

      <label className="text-sm font-medium text-slate-700">{t("المشروع")}</label>
      <input value={form.costCenterName} onChange={(e) => setField("costCenterName", e.target.value)} placeholder={t("اختياري")} className={inputClass} />
    </div>
  );
}
...
function InvoiceForm({ onBack, onSaved }: { onBack: () => void; onSaved: (i: PurchaseInvoice) => void }) {
  ...
  const handleSave = async () => {
    ...
    if (!form.date) { setError(t("يرجى إدخال تاريخ الفاتورة")); return; }
    if (!form.vendor.trim()) { setError(t("يرجى اختيار المورد قبل حفظ الفاتورة")); return; }
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : t("حدث خطأ غير متوقع أثناء الحفظ"));
    } finally {
    ...
    <div className="space-y-6" dir={direction}>
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">{t("فاتورة مشتريات")}</h1>
            <p className="text-sm text-slate-500">{t("معلومات فاتورة المشتريات")}</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-slate-100 px-4 py-2 text-right font-semibold text-slate-800">{t("معلومات فاتورة المشتريات")}</div>
      ...
      <div className="flex justify-center gap-4 pt-2">
        <button onClick={onBack} disabled={saving} className="px-6 py-2 bg-slate-500 text-white text-sm rounded hover:bg-slate-600 disabled:opacity-50">{t("إلغاء")}</button>
  ...
  return (
    <div className="space-y-6 bg-slate-50 min-h-screen pb-12" dir={direction}>
      <div className="flex justify-between items-center bg-white p-4 border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold text-slate-800">{t("تعديل فاتورة المشتريات")}</h1>
...
function InvoicePayment({ invoice, onBack, onUpdated }: { invoice: PurchaseInvoice; onBack: () => void; onUpdated: (i: PurchaseInvoice) => void }) {
  ...
  const handleSave = async () => {
    ...
    if (!error) {
      ...
      toast({ title: t("تم تسجيل الدفعة"), description: `${t("المبلغ")}: ${formatAmount(payAmount)} ${t("ريال")}` });
    } else {
  ...
  return (
    <div className="space-y-6 bg-slate-50 min-h-screen pb-12" dir={direction}>
      <div className="flex justify-between items-center bg-white p-4 border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold text-slate-800">{t("تسديد الفاتورة")}</h1>
      ...
      <div className="p-4 max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700 text-right block">{t("المبلغ المدفوع الآن")} <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-1">
              ...
              <input value={paymentRef} onChange={(e) => setPaymentRef(e.target.value)} placeholder={t("رقم المرجع...")} className="w-full px-3 py-2 border border-slate-300 rounded text-sm text-right focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none" />
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700 text-right block">{t("حالة الفاتورة")}</label>
        ...
        <div className="flex justify-center gap-4">
          <button onClick={onBack} className="px-6 py-2 bg-slate-500 text-white text-sm rounded hover:bg-slate-600">{t("إلغاء")}</button>
