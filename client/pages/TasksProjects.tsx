...
type Project = { id: string; name: string; description: string; status: string; project_type: string; department: string; manager_emp_id: string | null; manager_name: string; start_date: string | null; due_date: string | null; created_at: string };
type Task = { id: string; project_id: string | null; title: string; description: string; assignee_emp_id: string | null; assignee_name: string; status: string; priority: string; task_date: string | null; due_date: string | null; estimated_hours: number; progress: number; requires_approval: boolean; approval_status: string; rating: number | null; recurrence_type: string; additional_info: Record<string, unknown>; created_at: string };
...
type ModalName = "project" | "task" | "task-detail" | "project-detail" | "ticket" | "client" | "ticket-detail" | null;
...
export default function TasksProjects() {
  ...
  const [projectForm, setProjectForm] = useState({ name: "", projectType: "داخلي", department: "", managerId: "", description: "", startDate: today(), dueDate: "" });
  const [taskForm, setTaskForm] = useState({ title: "", description: "", projectId: "", assigneeId: "", collaboratorIds: [] as string[], priority: "متوسطة", taskDate: today(), dueDate: "", hours: "0", recurrenceType: "لا", pointsText: "", additionalInfo: "", files: [] as File[], requiresApproval: false });
  ...
  const uploadWorkFiles = async (entityType: "project" | "task" | "ticket", entityId: string, files: File[]) => {
    ...
    for (const file of files) {
      ...
      const uploadResult = await supabase.storage.from("work-files").upload(storagePath, file, { contentType: file.type, upsert: false });
      if (uploadResult.error) {
        toast.error(`تعذر رفع ${file.name}`);
        continue;
      }
      await supabase.from("work_attachments").insert({ entity_type: entityType, entity_id: entityId, file_name: file.name, storage_path: storagePath, mime_type: file.type || "application/octet-stream", file_size_bytes: file.size, uploaded_by: userId });
    }
  };
  ...
  const createProject = async () => {
    ...
    setSaving(true);
    const manager = employees.find((employee) => employee.emp_id === projectForm.managerId);
    const userId = (await supabase.auth.getUser()).data.user?.id;
    const { data: createdProject, error } = await supabase.from("work_projects").insert({ name: projectForm.name.trim(), project_type: projectForm.projectType, department: projectForm.department, manager_emp_id: manager?.emp_id || null, manager_name: manager?.name || "", description: projectForm.description.trim(), start_date: projectForm.startDate || null, due_date: projectForm.dueDate || null, status: "جديد", created_by: userId }).select("id").single();
    setSaving(false);
    if (error) return toast.error(error.message);
    if (createdProject) await supabase.from("work_activity_log").insert({ entity_type: "project", entity_id: createdProject.id, action: "تم إنشاء المشروع", actor_name: session?.name || "مستخدم", created_by: userId });
    setModal(null); setProjectForm({ name: "", projectType: "داخلي", department: "", managerId: "", description: "", startDate: today(), dueDate: "" }); await load(); toast.success("تم إنشاء المشروع");
  };
  ...
  const createTask = async () => {
    ...
    const { data: createdTask, error } = await supabase.from("work_tasks").insert({
      ...
      additional_info: taskForm.additionalInfo.trim() ? { notes: taskForm.additionalInfo.trim() } : {},
      status: taskForm.requiresApproval ? "بانتظار الموافقة" : "انتظار العمل", created_by: (await supabase.auth.getUser()).data.user?.id,
    }).select("id").single();
    if (!error && createdTask) await supabase.from("work_activity_log").insert({ entity_type: "task", entity_id: createdTask.id, action: "تم إنشاء المهمة", actor_name: session?.name || "مستخدم", created_by: (await supabase.auth.getUser()).data.user?.id });
    if (!error && createdTask && taskForm.pointsText.trim()) {
      const points = taskForm.pointsText.split("\n").map((point) => point.trim()).filter(Boolean);
      await supabase.from("work_task_points").insert(points.map((title, index) => ({ task_id: createdTask.id, title, sort_order: index + 1 })));
    }
    if (!error && createdTask && taskForm.files.length > 0) await uploadWorkFiles("task", createdTask.id, taskForm.files);
    ...
    setSaving(false);
    if (error) return toast.error(error.message);
    setModal(null); setTaskForm({ title: "", description: "", projectId: "", assigneeId: "", collaboratorIds: [], priority: "متوسطة", taskDate: today(), dueDate: "", hours: "0", recurrenceType: "لا", pointsText: "", additionalInfo: "", files: [], requiresApproval: false }); await load(); toast.success("تم إنشاء المهمة");
  };
  ...
  const updateTask = async (task: Task, patch: Partial<Task>) => {
    ...
    if (error) return toast.error(error.message);
    await supabase.from("work_activity_log").insert({ entity_type: "task", entity_id: task.id, action: "تم تحديث بيانات المهمة", details: patch, actor_name: session?.name || "مستخدم", created_by: (await supabase.auth.getUser()).data.user?.id });
    setTasks((current) => current.map((item) => item.id === task.id ? { ...item, ...patch } : item));
    if (selectedTask?.id === task.id) setSelectedTask({ ...selectedTask, ...patch });
  };
  ...
  const openTask = async (task: Task) => {
    ...
    const [pointsResult, commentsResult, attachmentsResult, activityResult] = await Promise.all([
      supabase.from("work_task_points").select("*").eq("task_id", task.id).order("sort_order"),
      supabase.from("work_comments").select("*").eq("entity_type", "task").eq("entity_id", task.id).order("created_at"),
      supabase.from("work_attachments").select("*").eq("entity_type", "task").eq("entity_id", task.id).order("created_at"),
      supabase.from("work_activity_log").select("*").eq("entity_type", "task").eq("entity_id", task.id).order("created_at", { ascending: false }),
    ]);
  ...
  const openProject = async (project: Project) => {
    ...
    const [commentsResult, attachmentsResult, activityResult] = await Promise.all([
      supabase.from("work_comments").select("*").eq("entity_type", "project").eq("entity_id", project.id).order("created_at"),
      supabase.from("work_attachments").select("*").eq("entity_type", "project").eq("entity_id", project.id).order("created_at"),
      supabase.from("work_activity_log").select("*").eq("entity_type", "project").eq("entity_id", project.id).order("created_at", { ascending: false }),
    ]);
    setDetailComments(commentsResult.data ?? []);
    setDetailAttachments(attachmentsResult.data ?? []);
    setDetailActivity(activityResult.data ?? []);
  ...
  const addDetailComment = async (entityType: "project" | "task", entityId: string) => {
    if (!detailComment.trim()) return;
    const userId = (await supabase.auth.getUser()).data.user?.id;
    const { error } = await supabase.from("work_comments").insert({ entity_type: entityType, entity_id: entityId, content: detailComment.trim(), author_name: session?.name || "مستخدم", created_by: userId });
    if (error) return toast.error(error.message);
    await supabase.from("work_activity_log").insert({ entity_type: entityType, entity_id: entityId, action: "تمت إضافة تعليق", actor_name: session?.name || "مستخدم", created_by: userId });
    setDetailComment("");
    if (entityType === "task" && selectedTask) await openTask(selectedTask);
    if (entityType === "project" && selectedProject) await openProject(selectedProject);
  };

  const togglePoint = async (point: any) => {
    await supabase.from("work_task_points").update({ completed: !point.completed }).eq("id", point.id);
    setDetailPoints((current) => current.map((item) => item.id === point.id ? { ...item, completed: !item.completed } : item));
  };

  const openAttachment = async (attachment: any) => {
    const { data, error } = await supabase.storage.from("work-files").createSignedUrl(attachment.storage_path, 300);
    if (error || !data?.signedUrl) return toast.error("تعذر فتح المرفق");
    ...
    <Layout>
      <div className="mx-auto flex min-h-[760px] max-w-[1650px] gap-5" dir="rtl">
        ...
        <main className="min-w-0 flex-1 space-y-5">
          ...
          {view === "projects" && <ProjectsView projects={projects} tasks={tasks} openProject={openProject} />}
          {["tasks", "daily", "collaborators", "my-collaboration", "approvals"].includes(view) && <TasksView tasks={filteredTasks} loading={loading} search={search} setSearch={setSearch} statusFilter={statusFilter} setStatusFilter={setStatusFilter} projectById={projectById} updateTask={updateTask} deleteTask={deleteTask} openTask={openTask} approvals={view === "approvals"} />}
      ...
      {modal === "project-detail" && selectedProject && <Modal title={selectedProject.name} onClose={() => setModal(null)} wide><div className="space-y-5"><div className="flex gap-2 overflow-x-auto border-b pb-3 text-xs font-bold text-slate-500"><span className="rounded-lg bg-cyan-50 px-3 py-2 text-cyan-700">نظرة عامة</span><span className="px-3 py-2">قائمة المهام</span><span className="px-3 py-2">مراحل المشروع</span><span className="px-3 py-2">مؤشرات المشروع</span><span className="px-3 py-2">الملفات</span><span className="px-3 py-2">التعليقات</span><span className="px-3 py-2">سير العمل والإنجاز</span></div><div className="grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-slate-900 p-5 text-white md:col-span-2"><div className="flex justify-between"><span className="rounded-full bg-white/10 px-2 py-1 text-xs">{selectedProject.project_type || "داخلي"}</span><span className="text-xs text-white/50">{selectedProject.status}</span></div><h3 className="mt-4 text-xl font-bold">{selectedProject.name}</h3><p className="mt-2 text-sm text-white/70">{selectedProject.description || "لا يوجد وصف"}</p><div className="mt-5 grid grid-cols-2 gap-3 text-xs"><div><span className="text-white/50">الإدارة</span><p className="mt-1 font-bold">{selectedProject.department || "-"}</p></div><div><span className="text-white/50">مدير المشروع</span><p className="mt-1 font-bold">{selectedProject.manager_name || "-"}</p></div><div><span className="text-white/50">من تاريخ</span><p className="mt-1">{selectedProject.start_date || "-"}</p></div><div><span className="text-white/50">إلى تاريخ</span><p className="mt-1">{selectedProject.due_date || "-"}</p></div></div></div><div className="flex flex-col items-center justify-center rounded-2xl border bg-cyan-50 p-5"><p className="text-xs font-bold text-cyan-700">نسبة التقدم</p><div className="mt-3 flex h-28 w-28 items-center justify-center rounded-full border-[12px] border-cyan-200 text-2xl font-black text-cyan-700">{(() => { const own = tasks.filter((task) => task.project_id === selectedProject.id); return own.length ? Math.round(own.filter((task) => task.status === "مكتملة").length / own.length * 100) : 0; })()}%</div></div></div><div className="rounded-2xl border"><div className="border-b p-3 font-bold">مهام المشروع</div><div className="divide-y">{tasks.filter((task) => task.project_id === selectedProject.id).map((task) => <button key={task.id} onClick={() => openTask(task)} className="flex w-full items-center justify-between p-3 text-right hover:bg-slate-50"><span>{task.title}</span><span className={`rounded-full px-2 py-1 text-xs ${statusTone[task.status] || "bg-slate-100"}`}>{task.status}</span></button>)}{tasks.filter((task) => task.project_id === selectedProject.id).length === 0 && <Empty label="لا توجد مهام في المشروع" />}</div></div><DetailFiles attachments={detailAttachments} openAttachment={openAttachment} onUpload={async (files) => { await uploadWorkFiles("project", selectedProject.id, files); await openProject(selectedProject); }} /><DetailComments comments={detailComments} value={detailComment} setValue={setDetailComment} onAdd={() => addDetailComment("project", selectedProject.id)} /><ActivityList activity={detailActivity} /></div></Modal>}
      {modal === "task-detail" && selectedTask && <Modal title="تفاصيل المهمة" onClose={() => setModal(null)} wide><div className="grid gap-5 lg:grid-cols-[280px_1fr]"><aside className="space-y-3 rounded-2xl bg-slate-100 p-4"><div><p className="text-xs text-slate-500">العنوان</p><p className="mt-1 font-black text-slate-900">{selectedTask.title}</p></div>{[["الأولوية", selectedTask.priority], ["الحالة", selectedTask.status], ["المشروع", selectedTask.project_id ? projectById.get(selectedTask.project_id) || "-" : "-"], ["المسؤول", selectedTask.assignee_name || "-"], ["تاريخ البداية", selectedTask.task_date || "-"], ["الموعد النهائي", selectedTask.due_date || "-"], ["التكرار", selectedTask.recurrence_type || "لا"]].map(([label, value]) => <div key={label} className="rounded-xl bg-white p-3"><p className="text-[10px] text-slate-400">{label}</p><p className="mt-1 text-sm font-bold">{value}</p></div>)}<div className="rounded-xl bg-white p-3"><p className="text-[10px] text-slate-400">عدد الساعات</p><p className="mt-1 text-2xl font-black text-cyan-700">{Number(selectedTask.estimated_hours || 0).toFixed(2)}</p></div><button onClick={() => updateTask(selectedTask, { status: "قيد التنفيذ" })} className="w-full rounded-xl bg-cyan-600 py-2 text-sm font-bold text-white">بدء تنفيذ المهمة</button><button onClick={() => { deleteTask(selectedTask); setModal(null); }} className="w-full rounded-xl border border-rose-200 py-2 text-sm text-rose-700">حذف المهمة</button></aside><div className="space-y-5"><div className="rounded-2xl border p-5"><div className="flex justify-between"><h3 className="font-bold">نسبة الإنجاز</h3><span className="text-2xl font-black text-cyan-700">{selectedTask.progress || 0}%</span></div><input type="range" min="0" max="100" step="5" value={selectedTask.progress || 0} onChange={(e) => { const progress = Number(e.target.value); setSelectedTask({ ...selectedTask, progress }); }} onMouseUp={() => updateTask(selectedTask, { progress: selectedTask.progress })} className="mt-4 w-full accent-cyan-600" /></div><div className="rounded-2xl border p-5"><h3 className="mb-3 font-bold">الوصف</h3><p className="whitespace-pre-wrap text-sm text-slate-600">{selectedTask.description || "لا يوجد وصف"}</p>{selectedTask.additional_info?.notes && <div className="mt-4 rounded-xl bg-blue-50 p-3 text-sm text-blue-800"><strong>معلومات إضافية: </strong>{String(selectedTask.additional_info.notes)}</div>}</div><div className="rounded-2xl border p-5"><h3 className="mb-3 font-bold">نقاط المهمة</h3><div className="space-y-2">{detailPoints.map((point) => <label key={point.id} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm"><input type="checkbox" checked={point.completed} onChange={() => togglePoint(point)} /> <span className={point.completed ? "text-slate-400 line-through" : ""}>{point.title}</span></label>)}{detailPoints.length === 0 && <p className="text-sm text-slate-400">لا توجد نقاط مضافة</p>}</div></div><DetailFiles attachments={detailAttachments} openAttachment={openAttachment} onUpload={async (files) => { await uploadWorkFiles("task", selectedTask.id, files); await openTask(selectedTask); }} /><DetailComments comments={detailComments} value={detailComment} setValue={setDetailComment} onAdd={() => addDetailComment("task", selectedTask.id)} /><ActivityList activity={detailActivity} /></div></div></Modal>}
...
function DetailFiles({ attachments, openAttachment, onUpload }: { attachments: any[]; openAttachment: (attachment: any) => void; onUpload: (files: File[]) => void }) { return <div className="rounded-2xl border p-5"><div className="mb-3 flex items-center justify-between"><h3 className="font-bold">المرفقات</h3><label className="cursor-pointer rounded-lg bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-700">إضافة ملفات<input type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.xls,.xlsx" className="hidden" onChange={(event) => onUpload(Array.from(event.target.files ?? []))} /></label></div><div className="grid gap-2 sm:grid-cols-2">{attachments.map((attachment) => <button key={attachment.id} onClick={() => openAttachment(attachment)} className="flex items-center gap-3 rounded-xl border bg-slate-50 p-3 text-right hover:bg-cyan-50"><FileBarChart className="h-7 w-7 text-emerald-600" /><span className="min-w-0"><span className="block truncate text-sm font-medium">{attachment.file_name}</span><span className="text-[10px] text-slate-400">{(Number(attachment.file_size_bytes || 0) / 1024).toFixed(1)} KB</span></span></button>)}{attachments.length === 0 && <p className="text-sm text-slate-400">لا توجد مرفقات</p>}</div></div>; }
function DetailComments({ comments, value, setValue, onAdd }: { comments: any[]; value: string; setValue: (value: string) => void; onAdd: () => void }) { return <div className="rounded-2xl border p-5"><h3 className="mb-3 font-bold">التعليقات</h3><div className="mb-3 max-h-52 space-y-2 overflow-y-auto">{comments.map((comment) => <div key={comment.id} className="rounded-xl bg-slate-50 p-3"><div className="flex justify-between text-[10px] text-slate-400"><span>{comment.author_name || "مستخدم"}</span><span>{new Date(comment.created_at).toLocaleString("ar-SA")}</span></div><p className="mt-2 text-sm">{comment.content}</p></div>)}{comments.length === 0 && <p className="text-sm text-slate-400">لا توجد تعليقات</p>}</div><div className="flex gap-2"><input value={value} onChange={(event) => setValue(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onAdd(); }} placeholder="اكتب تعليقًا..." className={inputClass} /><button onClick={onAdd} className="rounded-xl bg-cyan-600 px-4 text-white"><MessageSquare className="h-4 w-4" /></button></div></div>; }
function ActivityList({ activity }: { activity: any[] }) { return <div className="rounded-2xl border p-5"><h3 className="mb-3 font-bold">سجل الحركة</h3><div className="space-y-2">{activity.map((item) => <div key={item.id} className="flex items-center gap-3 text-sm"><span className="h-2 w-2 rounded-full bg-cyan-500" /><span className="flex-1">{item.action}</span><span className="text-[10px] text-slate-400">{new Date(item.created_at).toLocaleString("ar-SA")}</span></div>)}{activity.length === 0 && <p className="text-sm text-slate-400">سيظهر هنا تاريخ تغييرات العنصر</p>}</div></div>; }
